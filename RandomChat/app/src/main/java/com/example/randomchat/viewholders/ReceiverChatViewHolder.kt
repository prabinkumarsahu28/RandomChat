...
class ReceiverChatViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    fun setData(messages: Messages, receiverId: String) {
        itemView.apply {
            ...
            tvReceiverMessageChat.text = messages.message

            val time = Date(messages.timeStamp!!)
            val pre = SimpleDateFormat("KK:mm a");
            tvReceiverMessageTimeChat.text = pre.format(time).toString()

            clReceiverMessage.setOnLongClickListener {
